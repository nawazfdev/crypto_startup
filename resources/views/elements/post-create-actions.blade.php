<div class="d-flex flex-column flex-md-row">
    ...
    <div class="mt-1">
        ...
        >
            ...
            <span class="file-button-text">{{__("Files")}}</span>
        </span>
    </div>
    ...
    <div class="mt-1 ml-0 ml-md-2">
        <span
            ...
            data-action="{{!GenericHelper::isUserVerified() && getSetting('site.enforce_user_identity_checks') ? '' : 'price-dialog'}}"
...
$(document).ready(function() {
    $('.post-price-button:not(.disabled)').on('click', function(e) {
        e.preventDefault();
        e.stopPropagation();
        
        // Check if PostCreate is available
        if (typeof PostCreate !== 'undefined') {
            PostCreate.showSetPricePostDialog();
        }
    });
    
    // Prevent child elements from triggering parent events
    $('.post-price-button *').on('click', function(e) {
        e.stopPropagation();
        
        // Make sure we still trigger the price dialog
        if (typeof PostCreate !== 'undefined') {
            PostCreate.showSetPricePostDialog();
        }
    });
