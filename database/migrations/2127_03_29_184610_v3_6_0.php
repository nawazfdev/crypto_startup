...
class V360 extends Migration
{
    ...
    public function up()
    {
        ...
        if (Schema::hasTable('settings')) {
            ...
            // Order: 1130
            if (!DB::table('settings')->where('key', 'compliance.enable_age_verification_dialog')->exists()) {
...
"description" : "Can be generally used for denying user access for minors, adult content info, etc."
            ...
            if (!DB::table('settings')->where('key', 'compliance.age_verification_cancel_url')->exists()) {
                DB::table('settings')->insert([
