...
/**
 * Get image URL with fallback to placeholder if the image doesn't exist
 * 
 * @param string $path Image path
 * @param string $fallback Fallback image path
 * @return string Image URL
 */
function get_image_url($path, $fallback = '/img/default-avatar.png') {
    if (!$path) {
    ...
    if (Storage::disk(config('filesystems.defaultFilesystemDriver'))->exists($path)) {
