...
class PostsHelperServiceProvider extends ServiceProvider
{
    ...
    public static function getUserMediaTypesCount($userID)
    {
        ...
        // Check if the Stream model has the is_public column before using it in the query
        $streamsQuery = Stream::where('user_id', $userID)
            ->whereIn('status', [Stream::ENDED_STATUS, Stream::IN_PROGRESS_STATUS]);
        
        // Get the count of streams
        $streams = $streamsQuery->count();
