...
class MessengerController extends Controller
{
        ...
        Javascript::put([
            ...
            'user' => [
                ...
                'billingData' => [
                    ...
                    'credit' => Auth::user()->wallet ? Auth::user()->wallet->total : 0,
                ],
            ],
        ]);
    ...
    public static function selectizeList($q, $id)
    {
        ...
        // Filtering blocked users - using the safe approach to get blocked list ID
        $blockedUsers = [];
        if (Auth::check()) {
            $user = Auth::user();
            $blockedListId = null;
            if (method_exists($user, 'getBlockedListId')) {
                $blockedListId = $user->getBlockedListId();
            } else {
            ...
            if ($blockedListId) {
                $blockedUsers = ListsHelperServiceProvider::getListMembers($blockedListId);
            }
        }
    ...
    /**
     * Safely gets user lists IDs even if there's an issue with the User model
     * @return array
     */
    protected static function getSafeUserLists()
    {
        $user = Auth::user();
        $lists = [
            'blocked' => 0,
        ...
        // Get blocked list ID safely
        if (method_exists($user, 'getBlockedListId')) {
            $lists['blocked'] = $user->getBlockedListId() ?? 0;
        } else {
            $blockedList = \App\Model\UserList::where('user_id', $user->id)
                ->where('type', 'blocked')
                ->first();
            $lists['blocked'] = $blockedList ? $blockedList->id : 0;
        }
        
        // Get following list ID safely
        if ($user->lists) {
            $followingList = $user->lists->firstWhere('type', 'following');
            $lists['following'] = $followingList ? $followingList->id : 0;
        } else {
            $followingList = \App\Model\UserList::where('user_id', $user->id)
